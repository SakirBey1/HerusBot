...
def send_rules(update, chat_id, from_pm=False):
    ...
    try:
        ...
        if excp.message == "Sohbet bulunamadı" and from_pm:
            bot.send_message(user.id, "Bu sohbet için kurallar kısayolu doğru şekilde ayarlanmadı! Yöneticilerden şunu isteyin "
                                      "bunu tamir et.")
            return
        else:
            raise
...
def __stats__():
    return "{} sohbetlerin belirlenmiş kuralları vardır.".format(sql.num_chats())
...
__help__ = """
 - /rules: Bu sohbetin kurallarını al.

*Yalnızca yönetici:*
 - /setrules <buradaki kurallarınız>: bu sohbet için kuralları belirleyin.
