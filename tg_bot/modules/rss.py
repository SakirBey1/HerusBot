...
def show_url(bot, update, args):
    ...
    if len(args) >= 1:
        ...
        if link_processed.bozo == 0:
            feed_title = link_processed.feed.get("Başlık", default="Bilinmeyen")
            feed_description = "<i>{}</i>".format(
                re.sub('<[^<]+?>', '', link_processed.feed.get("Tanım", default="Bilinmeyen")))
            feed_link = link_processed.feed.get("Bağlantı", default="Bilinmeyen")

            feed_message = "<b>Besleme Başlığı:</b> \n{}" \
                           "\n\n<b>Besleme Açıklaması:</b> \n{}" \
                           "\n\n<b>Besleme Bağlantısı:</b> \n{}".format(html.escape(feed_title),
                                                               feed_description,
                                                               html.escape(feed_link))

            if len(link_processed.entries) >= 1:
                entry_title = link_processed.entries[0].get("Başlık", default="Bilinmeyen")
                entry_description = "<i>{}</i>".format(
                    re.sub('<[^<]+?>', '', link_processed.entries[0].get("Tanım", default="Bilinmeyen")))
                entry_link = link_processed.entries[0].get("Bağlantı", default="Bilinmeyen")
            ...
            update.effective_message.reply_text("Bu bağlantı bir RSS Beslemesi bağlantısı değildir")
    else:
...
def list_urls(bot, update):
    ...
    if len(final_content) == 0:
        bot.send_message(chat_id=tg_chat_id, text="Bu sohbet hiçbir bağlantıya abone değil")
    elif len(final_content) <= constants.MAX_MESSAGE_LENGTH:
        bot.send_message(chat_id=tg_chat_id, text="Bu sohbet aşağıdaki bağlantılara abonedir:\n" + final_content)
    else:
...
def add_url(bot, update, args):
    if len(args) >= 1:
        ...
        if link_processed.bozo == 0:
            ...
            if row:
                update.effective_message.reply_text("Bu URL zaten eklendi")
            else:
    ...
    if len(args) >= 1:
        ...
        if link_processed.bozo == 0:
            ...
            if user_data:
                ...
                update.effective_message.reply_text("Henüz bu URL'ye abone olmadınız")
        else:
            update.effective_message.reply_text("Bu bağlantı bir RSS Beslemesi bağlantısı değildir")
    else:
        update.effective_message.reply_text("URL eksik")
...
def rss_update(bot, job):
    ...
    for row in user_data:
        ...
        if len(new_entry_links) < 5:
            ...
            bot.send_message(chat_id=tg_chat_id, parse_mode=ParseMode.HTML,
                             text="<b>Uyarı: </b>{} istenmeyen postaları önlemek için olaylar dışarıda bırakıldı"
