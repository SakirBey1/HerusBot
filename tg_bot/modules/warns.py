...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    if is_user_admin(chat, user.id):
        message.reply_text("Lanet adminler, uyarılamaz bile!")
        return ""
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # kick
            ...
            reply = "{} uyarılar, {} tekmelendi!".format(limit, mention_html(user.id, user.first_name))
        ...
        reply = "{} vardır {}/{} uyarılar... dikkat!".format(mention_html(user.id, user.first_name), num_warns,
                                                             limit)
        if reason:
    ...
    if match:
        ...
        if res:
            update.effective_message.edit_text(
                "tarafından kaldırıldı uyarı {}.".format(mention_html(user.id, user.first_name)),
            ...
            update.effective_message.edit_text(
                "Kullanıcının zaten uyarısı yok.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML)
...
def warns(bot: Bot, update: Update, args: List[str]):
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            text = "Bu kullanıcı {}/{} uyarılar, aşağıdaki nedenlerle:".format(num_warns, limit)
            ...
            update.effective_message.reply_text(
                "kullanıcı {}/{} uyarılar, ancak hiçbiri için sebep yok.".format(num_warns, limit))
    else:
...
def remove_warn_filter(bot: Bot, update: Update):
    ...
    msg.reply_text("Bu geçerli bir uyarı filtresi değil - tüm aktif uyarı filtreleri için /warnlist komutunu çalıştırın.")
...
def set_warn_limit(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if args:
        if args[0].isdigit():
            if int(args[0]) < 3:
                msg.reply_text("Minimum uyarı limiti 3'tür!")
            else:
                sql.set_warn_limit(chat.id, int(args[0]))
                msg.reply_text("uyarı limiti güncellendi {}".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#SET_WARN_LIMIT" \
                       "\n<b>Admin:</b> {}" \
                       "\nUyarı sınırını şuna ayarlayın: <code>{}</code>".format(html.escape(chat.title),
...
def set_warn_strength(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        if args[0].lower() in ("üzerinde", "evet"):
            ...
            msg.reply_text("Çok fazla uyarı artık bir yasakla sonuçlanacak!")
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
                   "Güçlü uyarıları etkinleştirdi. Kullanıcılar engellenecektir.".format(html.escape(chat.title),
                                                                            mention_html(user.id, user.first_name))

        elif args[0].lower() in ("off", "no"):
            sql.set_warn_strength(chat.id, True)
            msg.reply_text("Çok fazla uyarı artık tekmeyle sonuçlanacak! Kullanıcılar daha sonra tekrar katılabilecekler.")
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
                   "Güçlü uyarıları devre dışı bıraktı. Kullanıcılar sadece atılacaktır.".format(html.escape(chat.title),
                                                                                  mention_html(user.id,
                                                                                               user.first_name))

        else:
            msg.reply_text("Sadece açık/evet/hayır/kapalı anlıyorum!")
    else:
        limit, soft_warn = sql.get_warn_setting(chat.id)
        if soft_warn:
            msg.reply_text("Uyarılar şu anda kullanıcıları limitleri aştıklarında *tekmelemek* için ayarlanmıştır.",
                           parse_mode=ParseMode.MARKDOWN)
        else:
            msg.reply_text("Uyarılar şu anda sınırları aştığında kullanıcıları *ban* olarak ayarlıyor.",
                           parse_mode=ParseMode.MARKDOWN)
    return ""


def __stats__():
    return "{} genel olarak uyarıyor {} sohpetler.\n" \
           "{} filtreleri uyar, genelinde {} sohpetler.".format(sql.num_warns(), sql.num_warn_chats(),
...
def __chat_settings__(chat_id, user_id):
    ...
    return "Bu sohbette `{}` filtreleri uyarın. Alır `{}` uyarır " \
           "kullanıcı almadan önce *{}*.".format(num_warn_filters, limit, "tekmelendi" if soft_warn else "yasak")


__help__ = """
 - /warns <userhandle>: bir kullanıcının numarasını ve uyarıların nedenini alın.
 - /warnlist: mevcut tüm uyarı filtrelerinin listesi

*Yalnızca yönetici:*
 - /warn <userhandle>: bir kullanıcıyı uyarır. 3 uyarıdan sonra kullanıcı gruptan uzaklaştırılır. Cevap olarak da kullanılabilir.
 - /resetwarn <userhandle>: bir kullanıcı için uyarıları sıfırlar. Cevap olarak da kullanılabilir.
 - /addwarn <anahtar kelime> <yanıt mesajı>: belirli bir anahtar kelime için bir uyarı filtresi ayarlayın. Anahtar kelimenizin \
bir cümle olsun, tırnak işaretleri içine alın: `/addwarn "çok kızgın" Bu kızgın bir kullanıcıdır`.
 - /nowarn <keyword>: bir uyarı filtresini durdur
 - /warnlimit <num>: uyarı sınırını ayarlayın
 - /strongwarn <açık/evet/kapalı/hayır>: Açık olarak ayarlanırsa, uyarı sınırının aşılması yasakla sonuçlanır. Aksi takdirde, sadece tekme atar.
"""
